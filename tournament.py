...
def run(board_game, human_turn, strong_heuristic):
    ...
    while not board_game.winner_found:
        ...
        if human_turn:
            ...
            elif len(actions) == 1 and boundChecker(actions[0]):
                print("User position input: {} is out of bound!".format(actions[0]))

            elif len(actions) == 2 and boundChecker(actions[0]) and boundChecker(actions[1]):
                print("User position input: {} is out of bound!".format(actions[1]))

            elif len(actions) == 1 and actions[0] is not None and board_game.addCounter > 0:

                valid = False

                actions[0] = actions[0][:3]  # This will trim the actions to remove trailing characters.
                if human_turn:
                    if(player1.tokenleft <= 0):
                        continue
                    valid = board_game.setTile(player1.symbol, actions[0])
                    if valid:
                        player1.tokenPlaced()
                    ...
                    if(player2.tokenleft <= 0):
                        continue
                    valid = board_game.setTile(player2.symbol, actions[0])
                    if valid:
                        player2.tokenPlaced()
                        human_turn = turn(human_turn)
            ...
            elif len(actions) == 2 and actions[0] is not None and actions[1] is not None and board_game.addCounter > 0:
                actions[0] = actions[0][:3]  # This will trim the actions to remove trailing characters.
                actions[1] = actions[1][:3]  # This will trim the actions to remove trailing characters.

                neighbours = board_game.showNeighbours(actions[0])
                if neighbourChecker(actions[1], neighbours[0]):
                    valid = board_game.moveTile(player1.symbol, actions[0], actions[1], human_turn)
                    if valid:
                        human_turn = turn(human_turn)
                else:
                    print("Can not move token from " + actions[0] + " to " + actions[1] + "!")
            else:
